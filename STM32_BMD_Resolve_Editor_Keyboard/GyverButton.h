...
class GButton {
  public:
	GButton(int8_t pin = BTN_NO_PIN,
          uint16_t debounce = DEBOUNCE,
          uint16_t new_timeout = NEW_TIMEOUT,
	        bool type = HIGH_PULL,
	        bool dir = NORM_OPEN); //!!!debounce = DEBOUNCE, new_timeout = NEW_TIMEOUT,
	        // конструктор кнопки, принимает PIN пин, тип type (HIGH_PULL / LOW_PULL) и направление dir (NORM_OPEN / NORM_CLOSE)
  ...
  uint8_t _PIN = 0;
	uint16_t _debounce = 0; //!!!60
	uint16_t _timeout = 0; //!!!300
